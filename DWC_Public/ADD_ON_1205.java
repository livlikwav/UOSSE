...
class MapDataManager { //맵에게 전달한 변수들 저장, 대부분 이름으로 역할 추정 가능
   ...
   Node GetCurrentPositionSensor(SIM sim, int x, int y, int dir) {
	   return psm.GetSensorPosition(sim, x, y, dir, mp.GetMapRow(), mp.GetMapCol());
   }
...
class PositionSensorManager{
   Node GetSensorPosition(SIM sim, int x, int y, int dir, int row, int col) {
